...
from homelab_ha_discovery.scripts import publish_podman_container_metrics as script


class PublishPodmanContainerMetricsTest(unittest.TestCase):
    def make_sample(
        self,
        *,
        timestamp: float,
    ...
    def test_discovery_topics_include_podman_scope_and_component(self) -> None:
        state_topic = script.podman_metrics_state_topic("hpc", "alice", "plex")

        with patch.object(script, "publish_mqtt_many") as publish_mqtt_many:
            script.publish_podman_container_discovery(
                "hpc",
                "alice",
                "plex",
                "plex",
        ...
        self.assertEqual(
            ...
            [
                (
                    ...
                    "homelab_ha_discovery_hpc_podman_alice_plex_state/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_health/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_restart_count/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_cpu_usage/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_memory_usage_mb/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_memory_limit_mb/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_memory_usage_percent/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_download_speed/config"
                ),
                (
                    "homeassistant/sensor/"
                    "homelab_ha_discovery_hpc_podman_alice_plex_upload_speed/config"
                ),
            ],
        )
        payloads = [
        ...
        self.assertEqual(payloads[0]["name"], "hpc Podman alice plex State")
        ...
        self.assertEqual(
            ...
            "homelab-ha-discovery_hpc_podman_alice_plex_metrics",
        )

    def test_scope_is_normalized_for_topics(self) -> None:
        self.assertEqual(script.podman_scope_from_value("Media User"), "media_user")
    ...
    def test_publish_state_payload_per_container(self) -> None:
        with (
            ...
            result = script.publish_podman_metrics_from_samples(
                (),
                "hpc",
                "root",
    ...
    def test_discovery_components_limits_discovery_not_state_publish(self) -> None:
        ...
        def sample(timestamp: float, offset: float) -> PodmanContainerSample:
            return PodmanContainerSample(
                containers=containers,
                stats={
                    plex_id: PodmanContainerStats(
        ...
        self.assertTrue(
            all("_podman_root_nginx_" in topic for topic in discovery_topics)
        )
        self.assertEqual(
            state_topics,
            [
                "homelab-ha-discovery/hpc/podman/root/nginx/metrics",
                "homelab-ha-discovery/hpc/podman/root/plex/metrics",
            ],
        )
    ...
    def test_debug_prints_sample_summary(self) -> None:
        ...
        self.assertRegex(
            stderr.getvalue(),
            r"DEBUG: \d{4}-\d{2}-\d{2}T.* calculated Podman metrics: metrics=1",
        )
