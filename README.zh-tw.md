...
目前可執行的發布器會從本機 Debian 主機、本機 Docker containers，以及透過 SSH 從 ASUS routers 收集指標，並將 JSON payload 發布到 MQTT。一般執行會先發布保留的 Home Assistant discovery config，然後發布目前的指標狀態。外部 systemd timer 執行可在 discovery 已註冊後使用 `--publisher-only`。也可以使用 `--timer SECONDS` 進入長時間執行的服務模式。
...
秒、network `1.0` 秒、Docker containers `60.0` 秒、ASUS router CPU
`1.0` 秒、ASUS router network `1.0` 秒，以及 ASUS router connected clients
`1.0` 秒。
...
service journal。產生的 `"expire_after": null` 會保留 timer-mode 預設，也就是
...
Timer mode 中，Docker discovery config 預設會將 `expire_after` 設為 timer 的三倍，
讓 Home Assistant 在錯過更新後將 stale container sensors 標記為 unavailable。
例如 `--timer 60` 預設會使用 `expire_after=180`。使用 `--expire-after 0` 可永不
expire，也可以傳入其他秒數：
