...
- Publisher cycles should batch discovery config and metric state messages through `publish_mqtt_many` so each cycle uses one MQTT connection.
...
- Docker container publishers should follow the same one-connection-per-cycle MQTT batching as other publishers.
