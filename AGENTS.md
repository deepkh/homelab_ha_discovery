...
- Each Python 3 script runs independently. The rough code flow is:
  ...
  - `src/homelab_ha_discovery/scripts/publish_frigate_metrics.py --ha-device-id <device> --url <url>` -> read Frigate Prometheus-style metrics over local HTTP from `/api/metrics`, parse system, camera, detector, GPU, and storage metrics, and publish one shared Frigate payload to the MQTT server. The default URL is `http://127.0.0.1:5000/api/metrics` and the publisher does not support username/password/auth arguments.
...
- Scripts that support `--timer SECONDS` run as long-running publishers with one persistent MQTT connection: publish immediately, then repeat every positive `SECONDS`, unless the metric requires a baseline sample such as network throughput.
...
- Scripts that support Home Assistant discovery should support `--expire-after SECONDS`. Reject negative, infinite, and NaN values. `--expire-after 0` means omit `expire_after`. In `--timer` mode, omitted `--expire-after` defaults to `--timer * 3`; in one-shot mode, omitted `--expire-after` means no expiry. Discovery config should write `expire_after` as an integer ceiling when effective.
- One-shot publisher runs should batch discovery config and metric state messages through `publish_mqtt_many` so each run uses one MQTT connection. Long-running `--timer` publishers should use `MqttPublisher` to keep one MQTT connection open and publish each cycle's batch through it.
- Discovery config publishes should use retain `True`.
...
- Docker container publishers should follow the same MQTT batching as other publishers: one connection for a one-shot run, and one persistent connection reused across long-running `--timer` cycles.
- Frigate metrics read Prometheus text from `http://127.0.0.1:5000/api/metrics` by default with no authentication options, validate HTTP failures, malformed text, and missing required metric families before publishing, and publish one shared state payload to `<prefix>/frigate/metrics/<device>`. The payload includes `system`, `cameras`, `detectors`, `gpus`, and `storage`. System metrics use only `pid="frigate.full_system"` from `frigate_cpu_usage_percent` and `frigate_mem_usage_percent`. Camera metrics use `camera_name` labels from `frigate_camera_fps`, `frigate_process_fps`, `frigate_skipped_fps`, and `frigate_detection_fps`. Detector metrics use `name` labels from `frigate_detector_inference_speed_seconds`. GPU metrics use `gpu_name` labels from `frigate_gpu_usage_percent` and `frigate_gpu_mem_usage_percent`. Storage metrics use `storage` labels from `frigate_storage_free_bytes` and `frigate_storage_used_bytes`; both byte values are converted to decimal GB and rounded to three decimal places.
- Frigate discovery topics include the `frigate` prefix and normalized dynamic component, for example `homeassistant/sensor/homelab_ha_discovery_<device>_frigate_system_cpu_usage/config`, `homeassistant/sensor/homelab_ha_discovery_<device>_frigate_camera_front_door_camera_fps/config`, `homeassistant/sensor/homelab_ha_discovery_<device>_frigate_detector_coral_inference_speed/config`, `homeassistant/sensor/homelab_ha_discovery_<device>_frigate_gpu_nvidia_0_usage/config`, and `homeassistant/sensor/homelab_ha_discovery_<device>_frigate_storage_media_frigate_recordings_free_gb/config`. Discovery value templates should read from the shared JSON payload. Units should be `%`, `fps`, `s`, and `GB` as appropriate.
...
- Debian host systemd detect/bootstrap should include a disabled `docker_containers` template entry with `include_label` defaulting to `homelab-ha-discovery.enabled=true`, `expire_after` defaulting to `null`, and timer `60.0`, but must not start Docker containers or publish MQTT messages during detection. Generated Docker unit names use `docker-containers`, for example `homelab-ha-discovery-hpc-docker-containers.service`. Docker entries may include `all`, `include_label`, `include_labels`, `docker_command`, `expire_after`, and `debug`. Any generated service entry may include `expire_after` to pass `--expire-after` to its publisher.
- Debian host systemd detect/bootstrap should probe `http://127.0.0.1:5000/api/metrics` with a short localhost HTTP timeout for Frigate only. If reachable, include an enabled `frigate` service entry with `url`, `timer: 10.0`, `expire_after: null`, and empty `missing_requirements`. If unreachable, include a disabled Frigate template with the same default URL and a note to enable after Frigate is running. Generated Frigate unit names use `frigate`, for example `homelab-ha-discovery-hpc-frigate.service`. Frigate entries may include `url`, `expire_after`, and `debug`.
